...
class FireworksProvider:
    def __init__(self):
        ...
        self.base_url = FIREWORKS_BASE_URL
    ...
    async def chat_completions(self, request_body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise Exception("Fireworks request timed out")
        except Exception as e:
            raise Exception(f"Fireworks request failed: {str(e)}")
        ...
        try:
            async with self.client.stream(
                ...
                if response.status_code != 200:
                    ...
                    raise Exception(f"Fireworks error {response.status_code}: {error_text.decode()}")
                
                async for line in response.aiter_lines():
                    if line.startswith("data: "):
                        yield f"{line}\n\n"
        except Exception as e:
            raise Exception(f"Fireworks stream failed: {str(e)}")
    ...
    async def health_check(self) -> tuple[bool, int, Optional[str]]:
        ...
        try:
            import time
            start = time.time()
            # Fireworks doesn't have a simple models endpoint accessible the same way
            # Use a lightweight completion as health check
            response = await self.client.post(
    ...
    def _mock_response(self, request_body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "id": f"chatcmpl-mock-fireworks-{uuid.uuid4().hex[:8]}",
            ...
            "choices": [{
                ...
                "message": {
                    "role": "assistant",
                    "content": f"[MOCK - Fireworks test mode] You said: '{last_message[:50]}...' Set FIREWORKS_API_KEY for real responses."
                },
                "finish_reason": "stop"
            }],
            "usage": {
    ...
    def _mock_stream_chunk(self, request_body: Dict[str, Any]) -> str:
        ...
        chunk = {
            ...
            "choices": [{
                "index": 0,
                "delta": {"role": "assistant", "content": "[MOCK - Fireworks test mode] Set FIREWORKS_API_KEY for real responses. "},
                "finish_reason": None
            }]
        }
