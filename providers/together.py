...
import json
from config import TOGETHER_API_KEY, TOGETHER_BASE_URL
...
class TogetherProvider:
    ...
    async def chat_completions(self, request_body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            raise Exception("Together AI request timed out")
        ...
        try:
            async with self.client.stream(
                ...
                if response.status_code != 200:
                    ...
                    raise Exception(f"Together AI error {response.status_code}: {error_text.decode()}")
                
                async for line in response.aiter_lines():
                    if line.startswith("data: "):
                        yield f"{line}\n\n"
        except Exception as e:
            raise Exception(f"Together AI stream failed: {str(e)}")
    ...
    def _mock_response(self, request_body: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "id": f"chatcmpl-mock-together-{uuid.uuid4().hex[:8]}",
            ...
            "choices": [{
                ...
                "message": {
                    "role": "assistant",
                    "content": f"[MOCK - Together AI test mode] You said: '{last_message[:50]}...' Set TOGETHER_API_KEY for real responses."
                },
                "finish_reason": "stop"
            }],
            "usage": {
    ...
    def _mock_stream_chunk(self, request_body: Dict[str, Any]) -> str:
        ...
        chunk = {
            ...
            "choices": [{
                "index": 0,
                "delta": {"role": "assistant", "content": "[MOCK - Together AI test mode] Set TOGETHER_API_KEY for real responses. "},
                "finish_reason": None
            }]
        }
